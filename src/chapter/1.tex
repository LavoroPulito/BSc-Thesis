...
\end{tabular}
\end{center}
\begin{lemma}[Anti-symmetry lemma]
...
\begin{definition}[Flow decompositon]
    ...
    To each path $P_i $ there is a corresponding  value $\phi_i \in \mb{N^+} | \phi_i > 0$ which is the value of the path flow.
...
\end{definition}
Note that the maximum number of path in our decomposition for any flow is $m$ sonce the capacities are integer.\\
\begin{center}
